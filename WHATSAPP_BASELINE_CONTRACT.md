...
| `services/whatsapp/preview_normalization.py` | The canonical preview/last-message rules. `should_apply_last_message` and `should_apply_unread_count` are the **single decision points** for the `max()` vs. monotonic debate. | `build_last_message_summary`, `should_apply_last_message`, `should_apply_unread_count` |
...
| `utils/whatsapp-formatting.js` | `TYPE_PREVIEW_LABELS`, `normalizePreviewText`, `buildChatPreview`, `sanitizeChatForEmit`, `sanitizeOutboundEvent`, `resolveSyncState`. | — |
