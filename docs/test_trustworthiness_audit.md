...
- **Routers / Endpoints (`backend/app/api/v1/endpoints/`):** `leads.py`, `discovery.py`, `campaigns.py`, `campaign_groups.py`, `settings.py`, `blacklist.py`, `analytics.py`, `scraper.py`, `smart_outreach.py`.
- **Domain Services (`backend/app/services/`):** `LeadIngestService`, `PhoneService`, `TaxonomyRegistry`, `SearchPlanner`, `SmartMatchingService`, `SpintaxService`, `AntibanPolicy`, `CampaignRunner`, `OutreachManager`, `BusinessQualityEngine`.
- **Database Models (`backend/app/models/`):** `Lead`, `CampaignGroup`, `campaign_group_leads` (junction), `Campaign`, `Conversation`, `Message`, `Blacklist`, `SystemSettings`.
...
| ~~`test_whatsapp_adversarial.py`~~ | ~~`whatsapp_sender.py`~~ | ~~`INV-WA-001`, `INV-WA-002`~~ | ~~Early dispatches, broken routing priorities~~ | — | — | — | **REMOVED** |
| ~~`test_webhook_adversarial.py`~~ | ~~`whatsapp_cloud_webhook.py`~~ | ~~`INV-WEB-001`, `INV-WEB-002`~~ | ~~Unsigned payload ingestion, duplicate `wamid`~~ | — | — | — | **REMOVED** |
...
| ~~**INVARIANT C: Zero Early Send**~~ | ~~WhatsApp backend removed~~ | — | — | — | — |
| ~~**INVARIANT D: WhatsApp Routing Matrix**~~ | ~~WhatsApp backend removed~~ | — | — | — | — |
| ~~**INVARIANT E: Webhook Authentication**~~ | ~~WhatsApp backend removed~~ | — | — | — | — |
| ~~**INVARIANT F: Webhook Idempotency**~~ | ~~WhatsApp backend removed~~ | — | — | — | — |
| ~~**INVARIANT G: Unknown WhatsApp Contact**~~ | ~~WhatsApp backend removed~~ | — | — | — | — |
...
- ~~**WhatsApp Network Dispatchers**~~ — WhatsApp backend removed.
...
| ~~**WhatsApp Preview**~~ | ~~Spintax generation tested~~ | ~~Preview endpoint calling send method~~ | ~~`WhatsAppSpy` asserts `call_count == 0`~~ | **REMOVED** |
| ~~**Webhook Ingestion**~~ | ~~HMAC SHA-256 header verified~~ | ~~Empty `WHATSAPP_CLOUD_APP_SECRET` falling back to dev mode~~ | ~~Documented `ADV-SEC-01`~~ | **REMOVED** |
