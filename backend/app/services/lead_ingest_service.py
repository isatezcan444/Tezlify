...
from backend.app.services.lead_normalization import (
    LEAD_VALUE_COLS,
    truncate_column_value,
    build_initial_custom_data,
    build_lead_values,
    merge_into_existing_lead,
    compute_bulk_correlation_key,
)
...
class LeadIngestService:
    ...
    # Extracted to backend.app.services.lead_normalization for SRP & decoupled testing.
    # Re-exported here as static attributes / methods for 100% backward compatibility.
    _LEAD_VALUE_COLS = LEAD_VALUE_COLS
    _merge_into_existing = staticmethod(merge_into_existing_lead)
    _initial_custom_data = staticmethod(build_initial_custom_data)
    _build_lead_values = staticmethod(build_lead_values)
    _bulk_key = staticmethod(compute_bulk_correlation_key)
